...
Expr f_weak(Expr U, Expr UHat, ModelParams p, Expr longitude, Expr latitude)
{
  ...
  Expr IHat = UHat[1];
  Expr RHat = UHat[2];
  // Expr phi = deg2rad * longitude;
  Expr theta = deg2rad * (90 - latitude);
  Expr sine_theta = sin(theta);
...
int main(int argc, char** argv)
{
  try
  {
    ...
    Expr longitude = new CoordExpr(0); // x coordinates in mesh
    Expr latitude = new CoordExpr(1); // y coordinates in mesh
    // Expr xy = List(longitude, latitude);
